  ...
  if (!hasImage && editable) {
    return (
      <div
        {...getRootProps()}
        className={cn(
          'group relative z-0 flex h-full w-full cursor-pointer select-none flex-col items-center justify-center gap-y-2 overflow-hidden rounded-2xl border-2 border-border/50 border-dashed bg-card shadow-sm transition-transform duration-200 ease-in-out md:cursor-move'
        )}
      >
        <input {...getInputProps()} />
